...
export function CommandBar({
  ...
  const tasks = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("tasks")
        .select("id,title,project_id,projects(slug,name)")
        .ilike("title", `%${term}%`)
        .limit(6);
      return data ?? [];
    },
  ...
  const messages = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("messages")
        .select("id,body,channel_id,channels(name)")
        .ilike("body", `%${term}%`)
        .is("deleted_at", null)
        .limit(6);
      return data ?? [];
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects
      .filter((p: any) => p.name?.toLowerCase().includes(tt) || p.slug?.toLowerCase().includes(tt))
      .slice(0, 10);
  }, [projects, term]);
    ...
    <CommandDialog open={open} onOpenChange={onOpenChange}>
      ...
      <CommandList>
        ...
        <CommandGroup heading={t("cmd.navigate")}>
          ...
          <CommandItem onSelect={() => go("/my-work")}>
          ...
          <CommandGroup heading={t("cmd.recent")}>
            {recent.map((r) => (
              <CommandItem
                key={r.id}
                onSelect={() =>
                  go(r.entity_type === "project" ? `/projects/${r.label ?? r.entity_id}` : "/tasks")
                }
              >
        ...
        {(tasks.data?.length ?? 0) > 0 && (
          <CommandGroup heading={t("cmd.tasks")}>
            {tasks.data!.map((tk: any) => (
              <CommandItem
                key={tk.id}
                onSelect={() => go(tk.projects?.slug ? `/projects/${tk.projects.slug}` : "/tasks")}
              >
                <CheckSquare className="size-4 mr-2" />
