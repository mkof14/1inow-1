      ...
      <header className="mb-8 flex items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Truth-first AI grounded in your verified data. What it knows, what it asks, what it does
            — all under your control.
          </p>
        </div>
        <div className="hidden md:flex items-center gap-2 rounded-xl border bg-card px-3 py-2 text-xs text-muted-foreground">
          <BrandMark className="h-3.5 w-3.5" /> No invented facts. Confidence is shown on every
          answer.
        </div>
      </header>
...
function MemoryPanel() {
  ...
  const [form, setForm] = useState({
    type: "project_fact" as MemoryType,
    key: "",
    value: "",
    confidence: "medium" as Confidence,
    zone: "business" as (typeof ZONES)[number],
  });
  ...
  const update = useMutation({
    mutationFn: async ({
      ...
      id: string;
      patch: { status?: "active" | "paused" | "rejected" | "archived" };
    }) => {
    ...
    <div className="grid gap-6 md:grid-cols-[1fr_320px]">
      <Card className="p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Every memory has a source, confidence and zone. Edit, pause or remove anything.
        </p>
        <div className="mt-4 space-y-2">
          {data.length === 0 && (
            <EmptyHint label="No memories yet. Add one on the right, or correct 1inow in chat and accept the save." />
          )}
          {data.map((m) => (
                ...
                <div className="flex shrink-0 items-center gap-1 opacity-0 transition group-hover:opacity-100">
                  <Button
                    ...
                    onClick={() =>
                      update.mutate({
                        id: m.id,
                        patch: { status: m.status === "paused" ? "active" : "paused" },
                      })
                    }
                  >
      ...
      <Card className="p-5 h-fit">
        ...
        <div className="mt-3 space-y-3 text-sm">
          <div>
            ...
            <Select
              value={form.type}
              onValueChange={(v) => setForm({ ...form, type: v as MemoryType })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div>
            ...
            <Input
              ...
              placeholder="e.g. AGRON terminology"
            />
          </div>
          <div>
            ...
            <Textarea
              ...
              placeholder="The fact, preference, or correction"
              rows={3}
            />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setForm({ ...form, confidence: v as Confidence })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                value={form.zone}
                onValueChange={(v) => setForm({ ...form, zone: v as (typeof ZONES)[number] })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <Button
            className="w-full"
            disabled={!form.key || !form.value || add.isPending}
            onClick={() => add.mutate()}
          >
            Save memory
          </Button>
...
function AgentsPanel() {
  ...
  const { data = [] } = useQuery({
    queryKey: ["ai_agents"],
    queryFn: async () =>
      (await supabase.from("ai_agents").select("*").order("created_at", { ascending: false }))
        .data ?? [],
  });
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from("ai_agents")
        .insert({ ...draft, user_id: u.user.id, status: "proposed", min_confidence: "medium" });
      if (error) throw error;
    },
    onSuccess: () => {
      toast.success("Agent proposed");
      setDraft({ name: "", purpose: "", scope: "" });
  ...
  return (
    <div className="grid gap-6 md:grid-cols-[1fr_320px]">
      <Card className="p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Temporary, scoped helpers. Approve to activate. Revoke any time.
        </p>
        <div className="mt-4 space-y-2">
          {data.length === 0 && (
            <EmptyHint label="No agents yet. Examples: Legal Reviewer, Investor Materials, Project Risk, Follow-Up." />
          )}
          {data.map((a) => (
            <div key={a.id} className="rounded-lg border bg-card p-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  {a.scope && (
                    <div className="mt-1 text-xs text-muted-foreground">scope: {a.scope}</div>
                  )}
                </div>
                <div className="flex gap-1">
                  ...
                  {a.status === "active" && (
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => setStatus.mutate({ id: a.id, status: "revoked" })}
                    >
                      Revoke
                    </Button>
                  )}
        ...
        <div className="mt-3 space-y-3 text-sm">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="e.g. contracts only"
            />
          </div>
          <Button
            className="w-full"
            disabled={!draft.name || !draft.purpose}
...
function WorkflowsPanel() {
  ...
  const { data = [] } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("ai_workflows").select("*").order("created_at", { ascending: false }))
        .data ?? [],
  });
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        .from("ai_workflows")
        .insert({ name, description: desc, user_id: u.user.id });
      if (error) throw error;
    },
    onSuccess: () => {
      ...
      setName("");
      setDesc("");
      qc.invalidateQueries({ queryKey: ["ai_workflows"] });
    },
  });
  return (
    <div className="grid gap-6 md:grid-cols-[1fr_320px]">
      <Card className="p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Reusable procedures 1inow can run on request.
        </p>
        <div className="mt-4 space-y-2">
          {data.length === 0 && (
            <EmptyHint label="No workflows yet. Examples: Investor follow-up, Project launch, Contract review." />
          )}
          {data.map((w) => (
      ...
      <Card className="p-5 h-fit">
        ...
        <div className="mt-3 space-y-3 text-sm">
          ...
          <Button className="w-full" disabled={!name} onClick={() => create.mutate()}>
            Save workflow
          </Button>
        </div>
      </Card>
...
function RulesPanel() {
  ...
  const { data = [] } = useQuery({
    queryKey: ["ai_rules"],
    queryFn: async () =>
      (await supabase.from("ai_rules").select("*").order("created_at", { ascending: false }))
        .data ?? [],
  });
  ...
  const add = useMutation({
    ...
    onSuccess: () => {
      setRule("");
      qc.invalidateQueries({ queryKey: ["ai_rules"] });
    },
  });
  ...
  return (
    <Card className="p-5">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Plain-language instructions. E.g. "Never send messages on weekends." "Always ask before
        scheduling with investors."
      </p>
      <div className="mt-4 flex gap-2">
        <Input
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" && rule.trim()) add.mutate();
          }}
        />
        <Button disabled={!rule.trim()} onClick={() => add.mutate()}>
          Add
        </Button>
      </div>
      <div className="mt-4 space-y-2">
        ...
        {data.map((r) => (
          ...
          >
            <div className={r.active ? "text-sm" : "text-sm text-muted-foreground line-through"}>
              {r.rule}
            </div>
...
function QuestionsPanel() {
  ...
  const { data = [] } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("ai_questions").select("*").order("created_at", { ascending: false }))
        .data ?? [],
  });
  const answer = useMutation({
    mutationFn: async ({ id, ans }: { id: string; ans: string }) => {
      const { error } = await supabase
        .from("ai_questions")
        .update({ answer: ans, status: "answered", answered_at: new Date().toISOString() })
        .eq("id", id);
      if (error) throw error;
    },
  ...
  const dismiss = useMutation({
    mutationFn: async (id: string) => {
      await supabase.from("ai_questions").update({ status: "dismissed" }).eq("id", id);
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["ai_questions"] }),
  });
  ...
  return (
    <Card className="p-5">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        1inow asks only when context is missing or risk is high.
      </p>
      <div className="mt-4 space-y-2">
        {data.length === 0 && (
          <EmptyHint label="No open questions. 1inow will surface clarifications here when needed." />
        )}
        {data.map((q) => (
          <QuestionRow
            key={q.id}
            q={q}
            onAnswer={(ans) => answer.mutate({ id: q.id, ans })}
...
function QuestionRow({
  ...
  q: { id: string; question: string; status: string; answer: string | null };
    ...
    <div className="rounded-lg border bg-card p-3">
      ...
      {q.status === "open" ? (
        <div className="mt-2 flex gap-2">
          ...
          <Button disabled={!v.trim()} onClick={() => onAnswer(v)}>
            Reply
          </Button>
...
function QualityPanel() {
  ...
  return (
    <Card className="p-5">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Duplicates, missing owners, stale documents, conflicts. 1inow scans continuously.
      </p>
      <div className="mt-4 space-y-2">
          ...
          >
            <div>
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-muted-foreground">{i.subject_type}</span>
              </div>
              <div className="mt-1.5 text-sm">{i.description}</div>
              {i.suggested_fix && (
                <div className="text-xs text-muted-foreground">Suggested: {i.suggested_fix}</div>
              )}
            </div>
...
function PrivacyPanel() {
  ...
  const upsert = useMutation({
    mutationFn: async ({
      ...
      zone: string;
      patch: { enabled?: boolean; cross_zone_allowed?: boolean };
    }) => {
      const { data: u } = await supabase.auth.getUser();
      if (!u.user) throw new Error("not signed in");
      ...
      if (existing) {
        ...
        const { error } = await supabase
          .from("user_privacy_zones")
          .insert({ user_id: u.user.id, zone: zone as (typeof ZONES)[number], ...patch });
        if (error) throw error;
      }
    },
  ...
  return (
    <Card className="p-5">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Keep zones separated. 1inow cannot mix them unless you allow it.
      </p>
          ...
          return (
            <div key={z} className="rounded-lg border bg-card p-3">
              <div className="flex items-center justify-between">
                <div className="text-sm font-medium capitalize">{z}</div>
                <Switch
                  checked={enabled}
                  onCheckedChange={(v) => upsert.mutate({ zone: z, patch: { enabled: v } })}
                />
              </div>
              <div className="mt-2 flex items-center justify-between text-xs text-muted-foreground">
                ...
                <Switch
                  checked={cross}
                  onCheckedChange={(v) =>
                    upsert.mutate({ zone: z, patch: { cross_zone_allowed: v } })
                  }
                />
              </div>
            </div>
        ...
        <div className="mt-4 space-y-2">
          ...
          {actions.map((a) => (
            <div key={a.id} className="rounded-lg border bg-card p-3 text-sm">
              ...
              {a.prompt && (
                <div className="mt-1 text-xs text-muted-foreground line-clamp-2">{a.prompt}</div>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="mt-4 space-y-2">
          ...
          {confs.map((c) => (
            <div key={c.id} className="rounded-lg border bg-card p-3">
              <div className="flex items-center gap-2 text-xs">
                <ConfBadge c={c.confidence as Confidence} />
...
function PrefsPanel() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("assistant_preferences").select("*").maybeSingle()).data,
  });
  const save = useMutation({
    mutationFn: async (patch: Record<string, unknown>) => {
      ...
      const { error } = await supabase
        ...
        .upsert({ user_id: u.user.id, ...(data ?? {}), ...patch });
      if (error) throw error;
    },
    onSuccess: () => {
      toast.success("Saved");
      qc.invalidateQueries({ queryKey: ["assistant_prefs"] });
    },
  });
  return (
    <Card className="p-5">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Tune the personality, strictness and reach of 1inow.
      </p>
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <NumField
          label="Strictness (0–4)"
          value={data?.strictness ?? 2}
          onChange={(n) => save.mutate({ strictness: n })}
        />
        <NumField
        ...
        <NumField
          ...
          value={data?.notification_level ?? 2}
          onChange={(n) => save.mutate({ notification_level: n })}
        />
        <div className="flex items-center justify-between rounded-lg border p-3 sm:col-span-2">
          <div>
            <div className="text-sm font-medium">Memory enabled</div>
            <div className="text-xs text-muted-foreground">
              1inow remembers verified facts you save or accept.
            </div>
          </div>
          <Switch
  ...
  return (
    <div className="grid gap-6 md:grid-cols-[1fr_320px]">
      <Card className="p-5">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          1inow will surface these at the right moment. Snooze or dismiss anything.
        </p>
        <div className="mt-4 space-y-2">
          {data.length === 0 && (
            <EmptyHint label="No reminders yet. Add one on the right or ask 1inow to set one in chat." />
          )}
          {data.map((r) => (
      ...
      <Card className="p-5 space-y-3">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="Call partner about term sheet"
          />
        </div>
        <div className="space-y-2">
