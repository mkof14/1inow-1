...
import { fetchDecisions, fetchProjects, DECISION_STATUS_LABEL, type DecisionStatus, type DecisionImpact } from "@/lib/queries";
...
function DecisionsPage() {
  ...
  const [form, setForm] = useState({
    ...
    impact: "medium" as DecisionImpact, project_id: "none",
  });

  const create = useMutation({
    mutationFn: async () => {
      const { error } = await (supabase as any).from("decisions").insert({
        title: form.title, context: form.context, recommendation: form.recommendation,
        impact: form.impact, requested_by: user!.id,
        project_id: form.project_id === "none" ? null : form.project_id,
      });
      if (error) throw error;
    },
    onSuccess: () => {
      toast.success("Decision opened"); setOpen(false);
  ...
  const decide = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: DecisionStatus }) => {
      const { error } = await (supabase as any).from("decisions").update({
        status, decided_by: user!.id, decided_at: new Date().toISOString(),
      }).eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["decisions"] }); toast.success("Decision recorded"); },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1500px] mx-auto">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Impact</Label>
                  <Select value={form.impact} onValueChange={(v) => setForm({ ...form, impact: v as DecisionImpact })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{(["low","medium","high","critical"] as const).map(i => <SelectItem key={i} value={i}>{i}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
      ...
      <div className="grid gap-3">
        ...
        {rows.map((d: any) => (
          <div key={d.id} className="rounded-xl border border-border bg-card p-5 hover:border-accent/40 transition">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className={`size-2 rounded-full ${IMPACT_DOT[d.impact as DecisionImpact]}`} />
                  <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full border ${STATUS_STYLES[d.status as DecisionStatus]}`}>
                    {DECISION_STATUS_LABEL[d.status as DecisionStatus]}
                  </span>
                  {d.projects && (
                    <Link to="/projects/$slug" params={{ slug: d.projects.slug }}
                      className="text-[11px] text-muted-foreground hover:text-accent inline-flex items-center gap-1">
                      <span className="size-2 rounded-sm" style={{ background: d.projects.color ?? "#0a2540" }} />
                      {d.projects.name}
                    </Link>
                  )}
