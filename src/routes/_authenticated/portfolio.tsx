...
function PortfolioPage() {
  ...
  const visible = useMemo(() => {
    ...
    if (filter === "active") return list.filter((p: any) => p.status === "active" || p.status === "in_progress");
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      avgProgress: list.length ? Math.round(list.reduce((a: number, p: any) => a + (p.progress ?? 0), 0) / list.length) : 0,
    };
  }, [projects.data]);
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-[1500px] mx-auto fade-rise">
      ...
      <div className="surface-aurora shimmer-border rounded-3xl p-6 sm:p-10 mb-8 relative overflow-hidden">
        <div className="absolute -right-10 -top-10 text-accent/10 pointer-events-none"><BrandMark size={300} /></div>
        <div className="relative">
          ...
          <h1 className="font-display text-3xl sm:text-5xl font-semibold tracking-tight mb-3 max-w-3xl">{t("portfolio.title")}</h1>
          <p className="text-muted-foreground max-w-2xl text-sm sm:text-base leading-relaxed">{t("portfolio.subtitle")}</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-8 max-w-3xl">
            {[
              ...
              <div key={s.label} className="rounded-xl border border-border bg-card/60 backdrop-blur p-3">
          ...
          {([
            ...
            <button key={id} onClick={() => setFilter(id as any)}
              className={`px-3 h-8 rounded-md text-xs font-medium transition ${
                filter === id ? "bg-accent text-accent-foreground" : "text-muted-foreground hover:text-foreground"
              }`}>{label}</button>
          ))}
        </div>
        <Link to="/projects" className="text-xs text-muted-foreground hover:text-accent inline-flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3 mb-4">
                <div className="size-11 rounded-xl border border-border grid place-items-center" style={{ background: `${color}20`, color }}>
                  <PortfolioCard size={22} />
                </div>
                <span className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full border ${STATUS_TONE[p.status] ?? STATUS_TONE.archived}`}>
                  {(p.status as string).replace("_", " ")}
                </span>
              </div>
              <h3 className="font-display text-lg font-semibold mb-1 group-hover:text-accent transition-colors">{p.name}</h3>
              ...
              <div className="mt-auto">
                <div className="flex items-center justify-between text-[10px] uppercase tracking-widest text-muted-foreground mb-1.5">
                  <span>{t("portfolio.progress")}</span><span className="font-mono">{progress}%</span>
                </div>
        ...
        {visible.length === 0 && (
          <div className="col-span-full text-center text-sm text-muted-foreground py-12">{t("portfolio.empty")}</div>
        )}
      </div>
    </div>
  );
