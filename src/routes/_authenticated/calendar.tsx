    ...
    <div className="p-4 sm:p-6 md:p-8 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4 sm:flex sm:flex-wrap sm:items-center sm:justify-between mb-8">
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          ...
          <div className="text-sm font-medium font-display min-w-[100px] sm:min-w-[120px] text-center">
            {monthLabel}
          </div>
          <button
            onClick={() => {
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-7 xl:gap-3 2xl:gap-4">
          {days.map((d) => {
            ...
            return (
              <div
                key={key}
                className={`rounded-2xl border min-h-[220px] p-3 2xl:p-4 transition-all duration-300 ${
                  isToday
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-3 sm:mb-4">
                  <span
                    className={`min-w-0 rounded-full px-2 py-1 text-[10px] sm:text-xs uppercase tracking-widest font-medium ${isToday ? "bg-accent/15 text-accent" : "bg-muted/60 text-muted-foreground"}`}
                  >
                    {d.toLocaleDateString(undefined, { weekday: "short" })}
                  </span>
                  <span
                    className={`shrink-0 text-xl sm:text-2xl font-display font-semibold ${isToday ? "text-accent" : ""}`}
                  >
                    {d.getDate()}
                  </span>
                </div>
                <div className="space-y-2">
                  {items.map((t: any) => (
                    <div
                      key={t.id}
                      className="group text-xs px-2.5 py-2.5 2xl:px-3 rounded-xl border border-border/80 bg-background/85 hover:border-accent/50 hover:bg-accent/5 hover:shadow-sm transition-all duration-200 cursor-pointer"
                    >
                      ...
                      {t.projects && (
                        ...
                        >
                          <span
                            className="size-1.5 rounded-full shrink-0 bg-accent"
                            style={
                              t.projects.color ? { backgroundColor: t.projects.color } : undefined
                            }
                          />
