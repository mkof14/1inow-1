...
function Kpi({ icon: Icon, label, value, hint, tone = "accent" }: any) {
  const toneCls =
    tone === "danger" ? "text-rose-400" : tone === "success" ? "text-emerald-400" : "text-accent";
  return (
    <div className="surface-aurora shimmer-border rounded-2xl p-5">
      <div className="flex items-center justify-between mb-3">
        <div
          className={`size-9 rounded-xl bg-card/70 border border-border grid place-items-center ${toneCls}`}
        >
...
function ReportsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const T = allT.filter((x: any) => {
      if (statusFilter !== "all" && x.projects?.status) {
        return x.projects.status === statusFilter;
      }
      const stamp = x.completed_at
    ...
    const overdue = T.filter(
      (t: any) => t.due_date && t.status !== "done" && new Date(t.due_date) < new Date(),
    ).length;
    const completion = T.length ? Math.round((done / T.length) * 100) : 0;
    const avgProgress = P.length
      ? Math.round(P.reduce((a: number, p: any) => a + (p.progress ?? 0), 0) / P.length)
      : 0;

    const byStatus = Object.entries(
      P.reduce((a: Record<string, number>, p: any) => {
    ...
    const tasksByStatus = Object.entries(
      T.reduce((a: Record<string, number>, x: any) => {
    ...
    const spanDays = Math.max(
      1,
      Math.min(60, Math.round((toD.getTime() - fromD.getTime()) / 86400000) + 1),
    );
    ...
    const trend = days.map((d) => {
      ...
      const n = T.filter(
        (x: any) =>
          x.completed_at && new Date(x.completed_at) >= d && new Date(x.completed_at) < next,
      ).length;
    ...
    T.forEach((x: any) => {
      if (x.status === "done" && x.assignee_id)
        contributorMap[x.assignee_id] = (contributorMap[x.assignee_id] ?? 0) + 1;
    });
    ...
    const contributors = Object.entries(contributorMap)
      .map(([id, n]) => ({
        name: profById[id]?.full_name?.split(" ")[0] ?? profById[id]?.email?.split("@")[0] ?? "—",
        done: n,
      }))
    ...
    return {
      ...
      totalTasks: T.length,
      totalPeople: (people.data ?? []).length,
    };
      ...
      kpi: [
        {
          ...
          hint: `${stats.totalProjects} ${t("reports.kpi.total")}`,
        },
        {
          label: t("reports.kpi.atRisk"),
          value: String(stats.atRisk),
          hint: t("reports.kpi.healthHint"),
        },
        {
          label: t("reports.kpi.completion"),
          value: `${stats.completion}%`,
          hint: `${stats.done} / ${stats.totalTasks}`,
        },
        {
          label: t("reports.kpi.overdue"),
          value: String(stats.overdue),
          hint: t("reports.kpi.overdueHint"),
        },
        {
          label: t("reports.kpi.progress"),
          value: `${stats.avgProgress}%`,
          hint: t("reports.kpi.progressHint"),
        },
        {
          label: t("reports.kpi.people"),
          value: String(stats.totalPeople),
          hint: t("reports.kpi.peopleHint"),
        },
      ],
      sections: [
        {
          title: t("reports.export.section.projects"),
          head: ["Status", "Count"],
          rows: stats.byStatus.map((e: any) => [String(e.name), Number(e.value)]),
        },
        {
          title: t("reports.export.section.tasks"),
          head: ["Status", "Count"],
          rows: stats.tasksByStatus.map((e: any) => [String(e.name), Number(e.value)]),
        },
        {
          title: t("reports.export.section.velocity"),
          head: ["Day", "Completed"],
          rows: stats.trend.map((d) => [d.day, d.done]),
        },
        {
          title: t("reports.export.section.contributors"),
          head: ["Member", "Completed tasks"],
          rows: stats.contributors.map((c) => [c.name, c.done]),
        },
      ],
    };
  }
  ...
  const statuses = useMemo(
    () =>
      Array.from(
        new Set((projects.data ?? []).map((p: any) => p.status).filter(Boolean)),
      ) as string[],
    [projects.data],
  );
    ...
    <div className="p-4 sm:p-6 md:p-8 max-w-[1500px] mx-auto fade-rise">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        ...
        <Kpi
          ...
          value={stats.atRisk}
          hint={t("reports.kpi.healthHint")}
          tone={stats.atRisk > 0 ? "danger" : "success"}
        />
        <Kpi
          icon={CheckCircle2}
          label={t("reports.kpi.completion")}
            ...
            <PieChart>
              ...
              >
                {stats.byStatus.map((e, i) => (
                  <Cell key={i} fill={STATUS_COLORS[e.name as string] ?? "#64748b"} />
                ))}
              </Pie>
              <Tooltip
                contentStyle={{
            ...
            {stats.byStatus.map((e) => (
              <div key={e.name as string} className="flex items-center gap-1.5">
                <span
                  ...
                  style={{ background: STATUS_COLORS[e.name as string] ?? "#64748b" }}
                />
                <span className="capitalize text-muted-foreground">
                  {(e.name as string).replace("_", " ")}
                </span>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={stats.tasksByStatus}>
              ...
              <Bar dataKey="value" radius={[6, 6, 0, 0]}>
                {stats.tasksByStatus.map((e, i) => (
                  <Cell key={i} fill={TASK_COLORS[e.name as string] ?? "#64748b"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
