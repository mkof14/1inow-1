...
function EmailTemplatesPage() {
  ...
  const save = useMutation({
    mutationFn: (t: Editable) =>
      upsertEmailTemplate({
        ...t,
        variables:
          typeof (t as any).variables === "string"
            ? String((t as any).variables)
                .split(",")
                .map((s) => s.trim())
                .filter(Boolean)
            : (t.variables ?? []),
    ...
    <div className="p-6 space-y-4">
      ...
      <Dialog open={!!editor} onOpenChange={(o) => !o && setEditor(null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {editor?.id ? t("page.emails.editTitle") : t("page.emails.newTitle")}
            </DialogTitle>
          </DialogHeader>
          {editor && (
            <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setEditor({ ...editor, language: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={
                    Array.isArray(editor.variables)
                      ? editor.variables.join(", ")
                      : ((editor.variables as any) ?? "")
                  }
            ...
            <Tabs defaultValue="rendered">
              ...
              <TabsContent value="rendered" className="space-y-2">
                ...
                <div className="font-medium">
                  {renderTemplate(preview.subject, sampleVars(preview.variables))}
                </div>
                <div className="text-xs text-muted-foreground mt-3">Body</div>
                <div
                  className="rounded border border-border p-4 bg-background prose prose-sm max-w-none dark:prose-invert"
                  dangerouslySetInnerHTML={{
                    __html: renderTemplate(preview.body_html, sampleVars(preview.variables)),
                  }}
                />
              </TabsContent>
              ...
              <TabsContent value="vars">
                <ul className="text-sm space-y-1">
                  {(preview.variables ?? []).map((v) => (
                    <li key={v}>
                      <code className="bg-muted/50 px-1.5 py-0.5 rounded">{`{{${v}}}`}</code>
                    </li>
                  ))}
                  {(preview.variables ?? []).length === 0 && (
                    <li className="text-muted-foreground">No variables declared.</li>
                  )}
                </ul>
              </TabsContent>
            </Tabs>
