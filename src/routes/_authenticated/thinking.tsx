    ...
    <PageContainer>
      <SectionHeader
        title={
          <span className="inline-flex items-center gap-2">
            <ThinkingLoop size={22} /> {t("thinking.title", "Thinking Engine")}
          </span>
        }
        description={t(
          "thinking.subtitle",
          "Every request passes through 12 stages: understand, gather context, check rules, estimate confidence, plan — before any action is taken.",
        )}
      />
      ...
      <SafeCard className="mb-6">
        ...
        <Textarea
          ...
          placeholder={t(
            ...
            'e.g. "Prepare investor meeting for Project Atlas next Tuesday"',
          )}
          className="mt-2 min-h-[88px]"
        />
        <div className="mt-3 flex items-center justify-between gap-3">
          <Small>
            {t("thinking.note", "The engine never calls AI directly. It produces a plan first.")}
          </Small>
          <Button onClick={run} disabled={!prompt.trim()}>
            {t("thinking.run", "Run pipeline")}
          </Button>
        </div>
      </SafeCard>
      ...
      {result ? (
        ...
        <EmptyState
          ...
          description={t(
            "thinking.empty.desc",
            "Type something above and run the pipeline to see how the engine would respond.",
          )}
        />
      )}
    ...
    <ResponsiveGrid min={280}>
      <SafeCard>
        ...
        <div className="mt-2 flex flex-wrap gap-1">
          ...
          {r.understanding.verbs.slice(0, 4).map((v) => (
      ...
      <SafeCard>
        ...
        {r.missing.length ? (
          <ul className="mt-2 space-y-1">
            {r.missing.map((m) => (
              <li key={m}>
                <Small className="text-amber-600 dark:text-amber-400">— {m}</Small>
              </li>
            ))}
          </ul>
        ) : (
      ...
      <SafeCard>
        ...
        {r.selfCheck.conflicts.length === 0 && r.selfCheck.notes.length === 0 ? (
          ...
          <ul className="mt-2 space-y-1">
            {r.selfCheck.conflicts.map((c, i) => (
              <li key={`c${i}`}>
                <Small className="text-rose-600 dark:text-rose-400">⚠ {c}</Small>
              </li>
            ))}
            {r.selfCheck.notes.map((n, i) => (
              <li key={`n${i}`}>
                <Small>· {n}</Small>
              </li>
            ))}
      ...
      <SafeCard>
        ...
        <Small className="mt-1 block">
          Stamped: {new Date(r.log.createdAt).toLocaleTimeString()}
        </Small>
      </SafeCard>
    </ResponsiveGrid>
  );
