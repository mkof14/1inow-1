...
const statusVariants: Record<string, "default" | "secondary" | "outline" | "destructive"> = {
  draft: "secondary", sent: "default", accepted: "default",
  expired: "outline", canceled: "destructive", failed: "destructive",
};
...
function InvitationsPage() {
  ...
  const cancelInv = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-invitations"] }); toast.success("Canceled"); },
    onError: (e: any) => toast.error(e.message ?? "Failed"),
  });
  const resend = useMutation({
    mutationFn: resendInvitation,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-invitations"] }); toast.success("Resent"); },
    onError: (e: any) => toast.error(e.message ?? "Failed"),
  });
          ...
          <DialogContent>
            ...
            <div className="space-y-3 py-2">
              ...
              <div className="space-y-1.5">
                ...
                <Input id="inv-name" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Optional" />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Textarea id="inv-msg" value={message} onChange={(e) => setMessage(e.target.value)} rows={3} placeholder="Optional welcome message" />
              </div>
            </div>
            <DialogFooter>
              <Button variant="ghost" onClick={() => setOpen(false)}>Cancel</Button>
              <Button onClick={() => create.mutate()} disabled={!email || create.isPending}>
                <Mail className="size-4 mr-1.5" /> {create.isPending ? "Creating…" : "Send invitation"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
              ...
              {invitations.data?.map((inv) => (
                <tr key={inv.id} className="hover:bg-muted/20">
                  <td className="px-4 py-2.5">
                    ...
                    {inv.full_name && <div className="text-xs text-muted-foreground">{inv.email}</div>}
                  </td>
